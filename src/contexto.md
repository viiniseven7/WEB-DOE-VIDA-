...
### 1. Cadastro Geral
- ✅ **Telas "Teste de Elegibilidade" e "Cadastro e Agendamento"**:
  - **Componentes**: `EligibilityTestPage.tsx`, `RegistrationDonationPage.tsx`
  - ✅ **Fluxo implementado**: O `RegistrationDonationPage.tsx` realiza as seguintes chamadas em sequência:
    1. `POST /api/auth/register`: Para criar a conta do novo doador.
    2. `POST /api/auth/login`: Para autenticar o novo doador e obter um token.
    3. `POST /api/auth/agendamentos`: Para criar o agendamento usando o token.
  - ⚠️ **Importante:** Esta rota não pode ser misturada com a criação de outros perfis. Para criar Funcionários, Diretores ou Admins, utiliza-se a rota restrita `POST /api/auth/users`.

### 2. Login e Recuperação de Senha
- ✅ **Tela de Login**:
  - **Componente**: `LoginPage.tsx`
  - **Utiliza**: `POST /api/auth/login` para autenticar o usuário.
- ✅ **Recuperação de Senha**:
  - **Componentes**: `ForgotPasswordPage.tsx`, `ResetPasswordPage.tsx`
- ✅ **ROTAS IMPLEMENTADAS (via Supabase)**:
    - `POST /auth/forgot-password`: Envia o código de recuperação.
    - `POST /auth/reset-password`: Redefine a senha com o código.

### 3. Doador (`/dashboard/donor`)
- **Dashboard**: Consumir e exibir dados das doações do usuário.
  - **ROTA CONFIRMADA**: `GET /api/agendamentos` - Lista o histórico de agendamentos do usuário autenticado.
- **Agendamento**:
  - ✅ **Agendar doação**: `POST /api/auth/agendamentos` (Implementado em `RegistrationDonationPage.tsx`).
  - Confirmar Agendamento.
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/agendamentos/{id}/confirmar` | MÉTODO: PUT/PATCH | MOTIVO: Atualizar status do agendamento para confirmado pelo doador.*
- ✅ **Perfil**:
...
### 4. Funcionário (`/dashboard/staff`)
- **Dashboard**: Consumir do banco através de consultas (estatísticas, etc).
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/estatisticas/hemocentro` | MÉTODO: GET | MOTIVO: Obter dados consolidados (doações hoje, estoque baixo) para os cards iniciais.*
- ✅ **Agendamentos**:
...
- ✅ **Estoque**: Atualizar estoque e somar ml de acordo com as bolsas, informando descrição do processo.
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/estoque` | MÉTODO: PUT/POST | MOTIVO: Rota dedicada para dar entrada ou baixa no estoque de sangue.*
- **Busca de Doador**: Buscar somente doadores do *seu* hemocentro. Pode alterar tipo sanguíneo, telefone, hemocentro, inativar ou restringir usuário (`PUT /api/users/{id}`).
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/users?search={cpf_ou_nome}` | MÉTODO: GET | MOTIVO: Buscar dados de um doador específico.*
- **Perfil**: Editar o próprio cadastro (`PUT /api/users/{id}`).
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/users/{id}` | MÉTODO: PUT | MOTIVO: Atualizar dados próprios do funcionário. [AJUSTAR AMANHÃ - ROTA JÁ EXISTE]*

### 5. Diretor (`/dashboard/director`)
- **Dashboard**:
  - Remover Dashboard de Satisfação (se não usado).
  - Revisar estatísticas da equipe.
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/estatisticas/diretor` | MÉTODO: GET | MOTIVO: Obter indicadores de performance da equipe e do hemocentro.*
- ✅ **Gestão de Funcionários**:
  ...
  - Editar informações do funcionário (nome, cargo, telefone, status, permissões) (`PUT /api/users/{id}`).
    - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/users/{id}` | MÉTODO: PUT | MOTIVO: Alterar cargo, status ou dados do funcionário.*
  - Adicionar coluna/funcionalidade de criar e editar permissões usando Spatie Permissions.
    - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/permissoes` | MÉTODO: GET/POST/PUT | MOTIVO: Gerenciar as roles e permissões usando o Spatie.*
- **Estoque e Relatórios (Exportação em PDF)**:
  ...
  - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/relatorios/exportar` | MÉTODO: GET/POST | MOTIVO: Gerar e retornar o arquivo PDF ou dados consolidados para os relatórios.*

### 6. Administrador
- ✅ **Gestão Global**:
  - Cria campanhas e designa hemocentros para elas.
    - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/campanhas` | MÉTODO: POST/PUT/GET | MOTIVO: Gerenciar campanhas de doação e vincular aos hemocentros.*
  - ✅ Edita hemocentros e seus status (`PUT /api/auth/hemocentros/{hemocentro}`).
  - ✅ Pode criar doadores e editar/alterar permissões de usuários globais (`POST /auth/users`).

### 7. Hemocentro (Configurações)
- **Detalhes**:
  - Horário de funcionamento e datas disponíveis para agendar.
    - *SUGESTÃO DE ROTA - API LOCALHOST: `http://localhost:8000/api/auth/hemocentros/{hemocentro}/configuracoes` | MÉTODO: PUT | MOTIVO: Salvar dias úteis e horários disponíveis.*
...
### 8. Pendências (Próximas Reuniões)
